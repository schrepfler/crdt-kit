...
case class GCounterImpl(val shardId: java.util.UUID = UUID.randomUUID(), val payload: Map[java.util.UUID, Int] = Map.empty) extends GCounter[java.util.UUID, GCounterImpl, Int] {
  ...
  override def increment(amt: Int)(implicit commutativeMonoid: CommutativeMonoid[Int]): GCounterImpl = {
  ...
  override def value(implicit commutativeMonoid: CommutativeMonoid[Int]): Int = commutativeMonoid.combineAll(payload.withDefaultValue(0).valuesIterator)
